...
JSON_AUTO(Window, type, windowId, nativeHandle, name, transparent, resizable, maxWidth, maxHeight, layer, showAlways, dragBorderWidth, bufferName, rect, caption, scaleFactor,visible)
...
struct ScreenScaleCommand: public GMessage
{
    GMESSAGE_AUTO("command.screen.scale");
    std::uint32_t scale = 100;
};

JSON_AUTO(ScreenScaleCommand, type, scale)
...
JSON_AUTO(GameInputIntercept, type, intercepting, width, height)
...
JSON_AUTO(InGameWindowFocused, type, focusWindowId, posX, posY)

struct WindowVisible : public GMessage
{
    GMESSAGE_AUTO("window.visible");

    std::uint32_t windowId = 0;
    bool visible = true;
};

JSON_AUTO(WindowVisible, type, windowId, visible)
