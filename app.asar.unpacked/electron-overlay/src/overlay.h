  ...
  public:
    ...
    {
        {
            ...
            {
                std::cout << "err  createImageMem !!!!!!" << std::endl;
                ;
            }
    ...
    Napi::Value sendCommand(const Napi::CallbackInfo &info)
    {
        ...
        else if (command == "input.intercept")
        {
            ...
            std::cout << __FUNCTION__ << "  input.intercept" << message.intercept << std::endl;

            this->_sendMessage(&message);
        }
        else if(command == "screen.scale")
        {
            overlay::ScreenScaleCommand message;
            message.scale = commandInfo.Get("scale").ToNumber();
            this->_sendMessage(&message);
        }
    ...
    Napi::Value addWindow(const Napi::CallbackInfo &info)
    {
        ...
        message.layer = windowDetails.Get("layer").ToNumber();
        message.showAlways = windowDetails.Get("showAlways").ToBoolean();
        message.visible = windowDetails.Get("visible").ToBoolean();
        ...
        message.scaleFactor = windowDetails.Get("scaleFactor").ToNumber();
        ...
        int sprWidth = (int)(message.rect.width*message.scaleFactor);
        int sprHeight = (int)(message.rect.height*message.scaleFactor);
         createImageMem(message.windowId, message.bufferName, std::max(width, sprWidth), std::max(height, sprHeight));
    ...
    Napi::Value sendWindowBounds(const Napi::CallbackInfo& info)
    {
        ...
        float scaleFactor = info[2].ToNumber();

        auto& imageMem = shareMemMap_[message.windowId];

        int sprWidth = (int)(message.rect.width*scaleFactor);
        int sprHeight = (int)(message.rect.height*scaleFactor);
        if (sprWidth * sprHeight > imageMem->maxWidth * imageMem->maxHeight)
        {
            auto it = std::find_if(windows_.begin(), windows_.end(), [windowId = message.windowId](const auto &window) {
                return windowId == window.windowId;
            });
            ...
            createImageMem(window.windowId, window.bufferName, std::max(width, sprWidth), std::max(height, sprHeight));
    ...
    Napi::Value getInjectApp(const Napi::CallbackInfo &info)
    {
        Napi::Env env = info.Env();
    ...
    Napi::Boolean isGameForeground(const Napi::CallbackInfo &info)
    {
        Napi::Env env = info.Env();
        ...
        std::string a = info[0].ToString();
        std::string b = std::to_string(idProc);
        bool ret = a == b ? true: false;

        return Napi::Boolean::New(env, ret);
    }    

    Napi::Value sendChangVisible(const Napi::CallbackInfo& info)
    {
        Napi::Env env = info.Env();

        overlay::WindowVisible message;
        message.windowId = info[0].ToNumber();
        message.visible = info[1].ToBoolean();
        ...
        std::cout << "sendChangVisible  " << message.windowId <<"," << message.visible << std::endl;
    ...
    void notifyInputIntercepEvent(std::uint32_t pid, bool intercepting, std::uint32_t width, std::uint32_t height)
    {
        if (eventCallback_)
        {
            std::cout << "notifyInputIntercepEvent  " << width <<"," << height << std::endl;
    ...
    void notifyInGameWindowFocused(std::uint32_t pid, std::uint32_t windowId, std::uint32_t posX, std::uint32_t posY)
    {
        if (eventCallback_)
        {
            Napi::HandleScope scope(eventCallback_->env);
            ...
            object.Set("posX", Napi::Value::From(eventCallback_->env, posX));
            object.Set("posY", Napi::Value::From(eventCallback_->env, posY));
...
break;
    ...
    {
        node_async_call::async_call([this, pid, overlayMsg]() {
            notifyInputIntercepEvent(pid, overlayMsg->intercepting, overlayMsg->width, overlayMsg->height);
        });
    }
    ...
    {
        node_async_call::async_call([this, pid, overlayMsg]() {
            notifyInGameWindowFocused(pid, overlayMsg->focusWindowId, overlayMsg->posX, overlayMsg->posY);
        });
    }
};
