...
import {setOpscoreFeedbackIsOpen, setOpscoreFeedbackSent} from "../../redux/slices/common";
...
const OpScoreFeedbackModal = () => {
    const dispatch = useDispatch();
    const {t} = useTranslation();

    const { opscoreFeedbackIsOpen } = useTypedSelector(state => state.common);
    ...
    useEffect(() => {
        setIsOpen(opscoreFeedbackIsOpen);
    }, [opscoreFeedbackIsOpen])
    ...
    const onClickButton = () => {
        setGrade(0);
        setFeedback("");
        window.api.send("send-opscore-feedback", {
            stars: grade,
            content: feedback
        });
        setIsOpen(false);
        dispatch(setOpscoreFeedbackIsOpen(false));
        dispatch(setOpscoreFeedbackSent(true));
    };
    ...
    return (
        <Modal isOpen={isOpen}
            ...
            <div className="feedback-modal">
                <div className="feedback-modal__title">{t("opscore-feedback.title")}</div>
                ...
                <div className="feedback-modal__textarea" onSubmit={onSubmit}>
                    <textarea name="" id="" cols={30} rows={3} placeholder={t("opscore-feedback.placeholder")}
                              onChange={onChangeFeedback} value={feedback} />
                    <div className="feedback-modal__textarea-div" />
                    <button type="button" disabled={isDisabled} className="feedback-modal__textarea-submit" onClick={onClickButton}>{t("opscore-feedback.button")}</button>
                </div>
            </div>
...
export default OpScoreFeedbackModal;
